...
package com.code44.finance.ui.dialogs.recurrencepicker;
...
public class RecurrencePickerDialogFragment extends DialogFragment implements OnItemSelectedListener,
    ...
    private final Logger logger = Logger.with(RecurrencePickerDialogFragment.class.getSimpleName());
    ...
    private TextView mStartDatePreText;
    ...
    public void updateDialog() {
        ...
        final String startDateStr = DateUtils.formatDateTime(getActivity(), mModel.startDate.toMillis(false), DateUtils.FORMAT_ABBREV_MONTH | DateUtils.FORMAT_SHOW_YEAR);
        mStartDateTextView.setText(startDateStr);

        mEndSpinner.setSelection(mModel.end);
        if (mModel.end == RecurrenceModel.END_BY_DATE) {
            final String endDateStr = DateUtils.formatDateTime(getActivity(), mModel.endDate.toMillis(false), DateUtils.FORMAT_ABBREV_MONTH | DateUtils.FORMAT_SHOW_YEAR);
    ...
    private void doToast() {
        logger.error("Model = " + mModel.toString());
        String rrule;
        if (mModel.recurrenceState == RecurrenceModel.STATE_NO_RECURRENCE) {
            rrule = "Not repeating";
        } else {
    ...
    private void updateIntervalText() {
        if (mIntervalResId == -1) {
            mIntervalGroup.setVisibility(View.GONE);
            return;
        }
    ...
    private void updateStartDateText() {
        if (mStartDateResId == -1) {
            // TODO - invalid
            return;
        }

        mStartDatePreText.setText(mResources.getText(mStartDateResId));
    }
    ...
    private void updateEndCountText() {
        ...
        if (markerStart != -1) {
            if (markerStart == 0) {
                logger.error("No text to put in to recurrence's end spinner.");
            } else {
    ...
    @Subscribe public void onDateSet(DatePickerDialog.DateSelected dateSelected) {
        switch (dateSelected.getRequestCode()) {
            case START_DATE:
                if (mModel.startDate == null) {
                    mModel.startDate = new Time(mTime.timezone);
                    mModel.startDate.hour = mModel.startDate.minute = mModel.startDate.second = 0;
                }
                mModel.startDate.year = dateSelected.getYear();
                mModel.startDate.month = dateSelected.getMonthOfYear() - 1;
                mModel.startDate.monthDay = dateSelected.getDayOfMonth();
                mModel.startDate.normalize(false);
                updateDialog();
                break;
            ...
            case END_DATE:
                ...
                mModel.endDate.year = dateSelected.getYear();
                mModel.endDate.month = dateSelected.getMonthOfYear() - 1;
                mModel.endDate.monthDay = dateSelected.getDayOfMonth();
                mModel.endDate.normalize(false);
                updateDialog();
                break;
        }
    ...
    @Override
    public void onClick(View v) {
        if (mStartDateTextView == v) {
            mDatePickerDialog = (DatePickerDialog) getFragmentManager().findFragmentByTag(DatePickerDialog.FRAGMENT_DATE_PICKER);
            if (mDatePickerDialog != null) {
                mDatePickerDialog.dismiss();
            }
            DatePickerDialog.show(getFragmentManager(), START_DATE, mModel.startDate.toMillis(false));
        }
        else if (mEndDateTextView == v) {
            mDatePickerDialog = (DatePickerDialog) getFragmentManager().findFragmentByTag(DatePickerDialog.FRAGMENT_DATE_PICKER);
            if (mDatePickerDialog != null) {
                mDatePickerDialog.dismiss();
            }
            DatePickerDialog.show(getFragmentManager(), END_DATE, mModel.endDate.toMillis(false));
            ...
            mRecurrenceSetListener.onRecurrenceSet(mModel.startDate.toMillis(false), rrule);
            dismiss();
        }
    }

    public interface OnRecurrenceSetListener {
        void onRecurrenceSet(long dateStart, String rrule);
    }
